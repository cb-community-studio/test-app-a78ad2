...
const AdvantageDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    
    const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.name}</p>
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.address}</p>
    const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.address2}</p>
    ...
    return (
        <DataTable value={items} onRowClick={onRowClick} scrollable rowHover paginator rows={10} rowClassName="cursor-pointer">
            ...
            <Column field="address2" header="Address" body={pTemplate2} style={{ minWidth: "8rem" }} />
...
export default AdvantageDataTable;