...
import advantageFakerFactory from "./advantageFakerFactory";
...
const AdvantageFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = advantageFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("advantage").create(elem));
...
export default connect(null, mapDispatch)(AdvantageFakerDialogComponent);
